...
Top-3 listo para que otra capa implemente UI y persistencia del funnel.
"""
...
def _beneficio_principal(oferta: dict[str, Any]) -> str:
    ahorro = as_float(oferta.get("ahorro_pct"))
    gb = as_float(oferta.get("gb_incluidos"))
    bundle = str(oferta.get("descripcion_bundle") or "").strip()
    descripcion = str(oferta.get("descripcion_corta") or "").strip()
    if ahorro > 0:
        return f"Ahorro estimado de {ahorro:.0f}% frente a servicios por separado."
    if gb >= 9999:
        return "Incluye datos ilimitados."
    if gb > 0:
        return f"Incluye {_format_number(gb)} GB."
    if bundle:
        return f"Incluye {bundle}."
    return descripcion or "Oferta compatible con el perfil contractual del cliente."


def _razones_para_asesor(
    cliente: dict[str, Any], oferta: dict[str, Any]
) -> list[str]:
    """Construye razones descriptivas; no pretende reemplazar una explicación causal."""
    reasons: list[str] = []
    if es_oferta_mt(oferta) and as_bool(cliente.get("elegible_mt")):
        reasons.append("Tiene móvil postpago e internet hogar por separado y es elegible para MT.")

    ahorro = as_float(oferta.get("ahorro_pct"))
    if ahorro > 0:
        reasons.append(f"La oferta registra un ahorro estimado de {ahorro:.0f}% en el catálogo.")

    consumo = as_float(cliente.get("consumo_datos_gb_prom"))
    gb = as_float(oferta.get("gb_incluidos"))
    if consumo > 0 and gb >= 9999:
        reasons.append(f"Evita un límite fijo frente a un consumo promedio de {consumo:.1f} GB al mes.")
    elif consumo > 0 and gb >= consumo and gb > 0:
        reasons.append(f"La capacidad incluida cubre su consumo promedio de {consumo:.1f} GB al mes.")

    tipo = normalized(oferta.get("tipo_oferta"))
    segmento = normalized(oferta.get("segmento_objetivo"))
    if tipo == "upgrade" and segmento == "movil" and as_bool(cliente.get("tiene_movil")):
        reasons.append("Es compatible con su servicio móvil activo.")
    elif tipo == "upgrade" and segmento == "hogar" and as_bool(cliente.get("tiene_hogar")):
        reasons.append("Es compatible con su servicio hogar activo.")

    if not reasons:
        reasons.append("Es una de las ofertas compatibles con mayor propensión estimada.")
    return reasons[:3]


def _alertas_para_asesor(warnings: list[str]) -> list[str]:
    messages = {
        "POSIBLE_DOWNGRADE_PRECIO": "La cuota es menor que la referencia actual; confirmar que no pierda beneficios necesarios.",
        "MENOR_CAPACIDAD_DATOS": "La oferta incluye menos capacidad de datos que su referencia actual.",
    }
    return [messages[code] for code in warnings if code in messages]


def generar_speech_seguro(
    cliente: dict[str, Any], oferta: dict[str, Any], canal: str
) -> str:
    """Genera una plantilla comercial usando solo datos disponibles y verificables."""
    channel = normalized(canal).replace("_", " ")
    if channel == "call out":
        opening = "Hola. Le contacto de Movistar porque tenemos una alternativa que podría ajustarse a sus servicios."
        closing = "¿Le gustaría que revisemos juntos sus beneficios?"
    elif channel == "call in":
        opening = "Si desea revisar una alternativa comercial antes de finalizar, tenemos una opción compatible con sus servicios."
        closing = "¿Desea que le explique sus condiciones?"
    elif channel in {"whatsapp", "whatsapp asistido"}:
        opening = "Hola. Tenemos una alternativa compatible con sus servicios actuales."
        closing = "¿Le gustaría conocer los detalles?"
    elif channel in {"tienda", "tienda física", "tienda fisica"}:
        opening = "Si desea revisar opciones comerciales durante su visita, tenemos una alternativa para usted."
        closing = "¿Desea que revisemos sus beneficios?"
    else:
        opening = "Tenemos una alternativa comercial compatible con sus servicios."
        closing = "¿Desea conocer los detalles?"

    name = str(oferta.get("nombre_oferta") or "la oferta recomendada")
    price = as_float(oferta.get("precio_mensual"))
    detail_parts = [name]
    if price > 0:
        detail_parts.append(f"por S/ {price:.2f} al mes")
    ahorro = as_float(oferta.get("ahorro_pct"))
    if ahorro > 0:
        detail_parts.append(f"con un ahorro estimado de {ahorro:.0f}%")
    gb = as_float(oferta.get("gb_incluidos"))
    if gb >= 9999:
        detail_parts.append("con datos ilimitados")
    elif gb > 0:
        detail_parts.append(f"con {_format_number(gb)} GB incluidos")

    offer_text = ", ".join(detail_parts) + "."
    if es_oferta_mt(oferta) and as_bool(cliente.get("elegible_mt")):
        context_text = " Usted cuenta con móvil e internet hogar por separado y puede evaluar una alternativa Movistar Total."
    else:
        context_text = ""
    return f"{opening}{context_text} La recomendación es {offer_text} {closing}"


def construir_vista_asesor(
...
def generar_speech_rebate_seguro(
    ...
    """Prepara una transición breve sin presionar ni inventar condiciones."""
    motivo = normalized(motivo_rechazo)
    nombre = str(oferta.get("nombre_oferta") or "esta alternativa")
    precio = as_float(oferta.get("precio_mensual"))
    detalle_precio = f" por S/ {precio:.2f} al mes" if precio > 0 else ""
    if motivo == "precio":
        apertura = "Entiendo que el precio es importante."
    elif motivo in {"no_necesita", "ya_tiene_similar"}:
        apertura = "Entiendo que la primera opción no se ajusta a lo que necesita."
    else:
        apertura = "Entiendo su observación."
    return (
...
def preparar_candidatos(
    ...
    ofertas_no_disponibles = {
        normalized(value) for value in (contexto.get("ofertas_no_disponibles") or [])
    }
    restricciones_ofertas = contexto.get("restricciones_ofertas") or {}
    candidatos: list[dict[str, Any]] = []
    exclusiones: list[dict[str, Any]] = []

    for oferta in catalogo:
        evaluation = evaluar_reglas(cliente, oferta, contexto, oferta_actual)
        oferta_id_normalizada = normalized(oferta.get("oferta_id"))
        if oferta_id_normalizada in ofertas_bloqueadas:
            evaluation.exclude("OFERTA_EN_ESPERA_TRAS_RECHAZO")
        if oferta_id_normalizada in ofertas_no_disponibles:
            evaluation.exclude("OFERTA_NO_DISPONIBLE_OPERATIVAMENTE")
        restriction_codes = restricciones_ofertas.get(oferta.get("oferta_id")) or (
            restricciones_ofertas.get(oferta_id_normalizada)
        )
        if isinstance(restriction_codes, str):
            restriction_codes = [restriction_codes]
        for code in restriction_codes or []:
            evaluation.exclude(str(code))
...
def finalizar_recomendacion(
    ...
    roles = ["oferta_principal", "alternativa_1", "alternativa_2"]
        ...
        top3.append(
            {
                ...
                "vista_asesor": construir_vista_asesor(
                    cliente,
                    oferta,
                    canal,
                    role,
    ...
    if not decision["permite_recomendar"]:
        ...
        top3 = []
    elif decision["accion"] == "VALIDAR_CONSENTIMIENTO":
        status = "validar_consentimiento"
    elif ambiguedad_mt["requiere_pregunta"]:
        status = "requiere_preferencia_mt"
    else:
        status = "ok" if top3 else "sin_ofertas_elegibles"

    requiere_activar_oportunidad = normalized(canal).replace("_", " ") != "call out"
    acciones_durante_interaccion = (
        ["RESPONDER_PREFERENCIA_MT"]
        if ambiguedad_mt["requiere_pregunta"] and decision["permite_recomendar"]
        else (["PRESENTAR_OFERTA"] if top3 else [])
    )
    ...
    return {
        ...
        "top_3": top3,
        "flujo_ui": {
            "acciones_iniciales": decision["acciones_permitidas"],
            "requiere_activar_oportunidad_comercial": requiere_activar_oportunidad,
            "acciones_durante_interaccion_comercial": acciones_durante_interaccion,
            "acciones_despues_oferta": [
                "REGISTRAR_ACEPTACION",
                "REGISTRAR_OBJECION",
                "REGISTRAR_RECHAZO",
            ] if top3 else [],
            "mostrar_speech_automaticamente": (
                decision["accion"] == "CONTACTAR"
                and bool(top3)
                and not ambiguedad_mt["requiere_pregunta"]
            ),
        },
        "audit": {
            ...
            "fuentes_operativas_no_inferidas": [
                ...
                "ofertas_rechazadas_bloqueadas",
                "ofertas_no_disponibles",
                "restricciones_ofertas",
            ],
        },
    }
...
def recomendar_rebate(
    ...
    if not scored:
        return {
            "contract_version": "motor_frontend_v1",
            "accion": "sin_alternativa_compatible",
            "criterio": strategy["criterio"],
            "oferta": None,
            "decision_comercial": decision,
        }
    ...
    vista_asesor["speech"]["texto"] = generar_speech_rebate_seguro(
        chosen["oferta"], motivo_rechazo
    )
    return {
        "contract_version": "motor_frontend_v1",
        "accion": strategy["accion"],
